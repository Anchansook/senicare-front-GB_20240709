...
export default function CSWrite() {
    ...
    // event handler: 이미지 변경 이벤트 처리 함수 (썸네일 변경) //
    const onImageInputChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
        const { files } = event.target;
        if (!files || !files.length) return;

        const file = files[0];
        setProfileImageFile(file); // 파일 상태 유지
    ...
    useEffect(() => {
        const accessTonken = cookies[ACCESS_TOKEN];
        if (!accessTonken) return;

        getNurseListRequest(accessTonken).then(getNurseListResponse);
    }, []);
    ...
    useEffect(() => {
        ...
        // 계속 상태가 유지가 되기 때문에 언마운트 시 되돌리는 작업 필요 (return () => {})
    }, [modalOpen]);
