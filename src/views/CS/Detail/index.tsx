...
export default function CSDetail() {
    ...
    // state: 관리 기록 내용 상태 //
    const [recordContents, setRecordContets] = useState<string>('');
    // state: 사용 용품 개수 상태 //
    ...
    const getCareRecordListResponse = (responseBody: GetCustomerResponseDto | ResponseDto | null) => {
        const message = 
            !responseBody ? '서버에 문제가 있습니다.' :
            responseBody.code === 'VF' ? '잘못된 접근입니다.' :
            responseBody.code === 'AF' ? '잘못된 접근입니다.' :
    ...
    // effect: 고객 번호가 변경 시 고객 정보 요청 함수 //
    useEffect(() => {
        if (!customerNumber) return;
        const accessToken = cookies[ACCESS_TOKEN];
        if (!accessToken) return;
